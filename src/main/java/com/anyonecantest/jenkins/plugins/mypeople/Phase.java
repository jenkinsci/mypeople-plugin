...
import com.anyonecantest.jenkins.plugins.mypeople.model.JobState;
		...
		if (property != null) {
			...
			for (Endpoint target : targets) {
          		try {
               		...
               		if(jobState.getBuild().isFailure() == true) {
               			
               			String msg = MyPeople.getMessage(jobState.getBuild());
               			MyPeople.SetAPIKEY(target.getApikey());
               			boolean success = MyPeople.sendMessage(target.getBuddyId(), msg);
